...
namespace Wakatime
{
    namespace Internal
    {
        ...
        WakaOptionsWidget::WakaOptionsWidget(const QSharedPointer<WakaOptions> &options, QWidget *parent)
            : QWidget(parent), _ui(new Ui::WakaOptionsForm), _options(options)
        {
            _ui->setupUi(this);

            connect(_ui->shToolButton, &QToolButton::pressed, this,
                    [this]()
                    {
                        _ui->apiKeyLineEdit->setEchoMode(_ui->apiKeyLineEdit->echoMode() == QLineEdit::EchoMode::Password ? QLineEdit::EchoMode::Normal :
